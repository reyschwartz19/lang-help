...
- [x] Define and validate a prepared teaching-content import format with source attribution and deterministic content-release versions.
- [x] Import one prepared content release into PostgreSQL idempotently, without removing the current Dexie seed.
- [x] Add a read-only, paginated content route that returns release/version metadata and typed story/sentence payloads.
- [x] Add Dexie content-cache metadata and cache-ahead fetching while preserving offline reads and the existing Reader behavior.
...
- [x] Add Dexie `syncOutbox` and `syncMetadata` tables in a versioned local migration; local mutations must commit before enqueueing background work.
...
- [x] Add a typed `learner_events` schema and local-first event capture for real learning actions; do not invent or backfill metrics.
...
- [x] Fix sentence review to be genuinely audio-first: do not display the full French answer before reveal, and implement the specified target-word/phrase cloze behavior.
- [x] Keep phrase cards out of sentence review (or resolve them through a shared typed review-content abstraction) so queue rows cannot silently have missing content.
- [x] Make reading resurfacing honor `nextResurfaceAt` and the documented listen-only/reread sequence.
- [x] Remove avoidable `any` usage from reading-progress updates and validate all persisted date/status conversions.
...
- [x] Inventory every user-visible literal in `app/` and `components/`, classifying it as UI copy, bundled learning content, configuration, derived learner state, or obsolete mock data.
- [x] Remove the hard-coded header date and generate locale-aware dates only where a date provides real value.
- [x] Remove the hard-coded user name, avatar initial, level, streak, and profile details from `AppShell`; derive them from local settings/stats or show a neutral single-user state.
- [x] Replace hard-coded home-page XP, streak history, lesson progress, remaining lessons, review counts, activity durations, level, and featured-story details with real IndexedDB queries and derived selectors.
- [x] Replace the static `/progress` statistics, weekly chart, CEFR level, and motivational assessment with calculated learner events and honest empty states.
- [x] Replace or remove the static `/learn` chapter, lesson list, completion state, locks, and progress percentage until a real learning-path/session model exists.
- [x] Move reader stories, speaking scenarios, phrase categories, scenario categories, and other learning content out of route components into typed, validated content/configuration modules or prepared data assets.
...
- [x] Remove placeholder controls and claims that have no behavior, including Settings, Add phrase, fake goals, fake time estimates, and unsupported progress promises; alternatively implement and test the behavior.
- [x] Remove scaffold metadata and placeholder branding/assets that are not part of Parlez.
...
- [x] Establish CI for typecheck, lint, tests, and production build.
- [x] Remove dual npm/pnpm lockfile ambiguity and document the chosen package manager.
